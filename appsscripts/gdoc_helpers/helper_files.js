...
function isModified(next, opts) {
    ...
    try {
        ...
        console.error('watchFiles PERMISSION ERROR', next.getId())
        return
    }
...
function publishFile(opts) {
    ...
    if (opts.fileId) {
        ...
        if (!file.hasNext()) {
            return console.error('publishFile NO SUCH FILE', 'File', opts)
        }
    ...
    if (file.getOwner().getEmail() != 'webform@goodpill.org') {
        console.error('publishFile WRONG OWNER', 'File', file.getName(), 'Active User', Session.getActiveUser().getEmail(), 'Effective User', Session.getEffectiveUser().getEmail(), 'File Owner', file.getOwner().getEmail())
...
function moveFile(opts, retry) {
    ...
    console.warning('gdoc_helpers moveFile NO FILE #' + (retry ? 2 : 1) + ' of 2', opts)
}
