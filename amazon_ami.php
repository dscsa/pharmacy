    ...
    if (count($patient_profile)) {
      ...
      foreach ($patient_profile as $i => $rx) {
        $table .= "<tr style='font-size:14px'><td>".substr($patient_profile[$i]['drug_name'], 1, -1)."</td><td>".
          ...
          woocommerce_form_field("autofill_resume[".$patient_profile[$i]['rx_id']."]", ['custom_attributed' => ['nextFill' => date_format(date_create($patient_profile[$i]['refill_date']), 'm/d')], 'input_class' => ['date-picker'], 'return' => true])."</td><td style='font-size:16px'>".
