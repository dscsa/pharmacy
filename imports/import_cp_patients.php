...
function import_cp_patients() {
  ...
  $patients = $mssql->run("
    ...
    SELECT
      ...
      MAX(CASE WHEN Dam_agcsp = 900388 AND cppat_alr.status_cn <> 3 THEN 'No Known Allergies' ELSE NULL END) as allergies_none,
      MAX(CASE WHEN Dam_agcsp = 478 AND cppat_alr.status_cn <> 3 THEN 'Tetracyclines' ELSE NULL END) as allergies_tetracycline,
      MAX(CASE WHEN Dam_agcsp = 477 AND cppat_alr.status_cn <> 3 THEN 'Cephalosporins' ELSE NULL END) as allergies_cephalosporins,
      MAX(CASE WHEN Dam_agcsp = 491 AND cppat_alr.status_cn <> 3 THEN 'Sulfa' ELSE NULL END) as allergies_sulfa,
      MAX(CASE WHEN hic ='H3DB' AND cppat_alr.status_cn <> 3 THEN 'Aspirin' ELSE NULL END) as allergies_aspirin,
      MAX(CASE WHEN Dam_agcsp = 476 AND cppat_alr.status_cn <> 3 THEN 'Penicillin' ELSE NULL END) as allergies_penicillin,
      MAX(CASE WHEN hic ='W1DA' AND cppat_alr.status_cn <> 3 THEN 'Erythromycin' ELSE NULL END) as allergies_erythromycin,
      MAX(CASE WHEN hic ='H3AH' AND cppat_alr.status_cn <> 3 THEN 'Codeine' ELSE NULL END) as allergies_codeine,
      MAX(CASE WHEN Dam_agcsp = 439 AND cppat_alr.status_cn <> 3 THEN 'NSAIDS' ELSE NULL END) as allergies_nsaids,
      MAX(CASE WHEN Dam_agcsp = 270 AND cppat_alr.status_cn <> 3 THEN 'Salicylates' ELSE NULL END) as allergies_salicylates,
      MAX(CASE WHEN hic ='W1DH' AND cppat_alr.status_cn <> 3 THEN 'Azithromycin' ELSE NULL END) as allergies_azithromycin,
      MAX(CASE WHEN hic ='W1AU' AND cppat_alr.status_cn <> 3 THEN 'Amoxicillin' ELSE NULL END) as allergies_amoxicillin,
      MAX(CASE WHEN hic ='' AND Dam_agcsp = 0 AND cppat_alr.status_cn <> 3 THEN name ELSE NULL END) as allergies_other,
