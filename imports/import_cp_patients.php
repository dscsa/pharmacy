...
function import_cp_patients() {
  ...
  $patients = $mssql->run("
    ...
    SELECT
      ...
      MAX(SUBSTRING(pat.cmt, 0, ISNULL(NULLIF(CHARINDEX(CHAR(10)+'___', pat.cmt), 0), DATALENGTH(pat.cmt)+1))) as patient_note,
      MAX(RIGHT(CAST(pat.cmt as VARCHAR(3072)), DATALENGTH(pat.cmt)-ISNULL(NULLIF(CHARINDEX('___'+CHAR(13), pat.cmt)+3, 3), DATALENGTH(pat.cmt)))) as medications_other,
