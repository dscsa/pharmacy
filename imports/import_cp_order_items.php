...
function import_cp_order_items() {
  ...
  $items = $mssql->run("
    ...
    WHERE line_state_cn < 50 OR (cprx.chg_date > (@today - 1))-- Unshipped only to cut down volume. line_state < 50 by itself did not enable qty/days_dispensed_actual to get set properly
