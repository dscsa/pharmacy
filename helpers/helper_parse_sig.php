...
function clean_sig($sig) {
  ...
  $sig = preg_replace('/.*(exceed (more than )?|exceeding |every \d minutes for |totaling |up ?to |total of |not to |max(imum)? (of |per day (of )?|per day dose( |: ?)|daily dose( |: ?))?)([0-9]*\.[0-9]+|[1-9][0-9]*)/i', 'Max $8', $sig); //Get rid of "max" qtys in sig because they are redundant and could accidentally be added in
