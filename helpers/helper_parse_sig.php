...
function qtys_per_time($durations, $drug_name, $correct) {
  ...
  foreach ($durations as $sig_part => $duration) {
    ...
    $count = preg_match_all('/([0-9]*\.[0-9]+|[1-9][0-9]*) ?(ml|tab|cap|pill|softgel|patch|injection|each)|(^|use +|take +|inhale +|chew +|inject +|oral +)([0-9]*\.[0-9]+|[1-9][0-9]*)(?!\d| *\.| *mg| *time)/i', $sig_part, $match);
