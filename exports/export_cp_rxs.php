...
function export_cp_rx_autofill($item) {
  ...
  $sql = "UPDATE cprx SET autofill_yn = $item[rx_autofill], chg_date = GETDATE() WHERE pat_id = $item[patient_id_cp] AND script_no IN ('$rx_numbers')";
  $mssql->run($sql);
}
